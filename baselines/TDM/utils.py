...
class MLP_Net(nn.Module):
    def __init__(self, d):
        ...
        self.lin3 = nn.Linear(d, 1)
        self.relu = nn.ReLU()

    def forward(self, x):
        x = torch.nn.functional.linear(x, self.lin1.weight.clone(), self.lin1.bias)
        x = self.relu(x)
        x = torch.nn.functional.linear(x, self.lin2.weight.clone(), self.lin2.bias)
        x = self.relu(x)
        x = torch.nn.functional.linear(x, self.lin3.weight.clone(), self.lin3.bias)
        return x
