...
def load_dataset(dataname, idx = 0, mask_dir=None, DATA_DIR=DATA_DIR):
    ...
    if train_mask.dtype == 'float32':
        train_mask = train_mask.astype(bool)
    if test_mask.dtype == 'float32':
...
def get_eval(dataname, X_recon, X_true, truth_cat_idx, num_num, cat_bin_num, mask, DATA_DIR=DATA_DIR):
...
def get_subset_idx(total_num, target_num=[1000, 6000, 11000, 16000]):
    target_num_reversed = target_num[::-1]
    ret = {}
    row_num = total_num
    for i, num in enumerate(target_num_reversed):
        row_idx = np.random.choice(row_num, num ,replace=False)
        ret[target_num_reversed[i]] = row_idx
        row_num = len(row_idx)
    
    ret['all'] = np.arange(total_num) #for compatibility
