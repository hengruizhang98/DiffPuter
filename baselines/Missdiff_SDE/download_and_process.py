...
NAME_URL_DICT_UCI = {
    ...
    # 'minibooe': 'https://archive.ics.uci.edu/static/public/199/miniboone+particle+identification.zip',
...
def process_letter():
    dataname = 'letter'
    path = f'data/{dataname}/{dataname}-recognition.data'
    save_path = f'data/{dataname}/data.csv'
...
def process_news():
    path = f'data/news/OnlineNewsPopularity/OnlineNewsPopularity.csv'
    save_path = 'data/news/data.csv'
...
def process_beijing():
    path = f'data/beijing/PRSA_data_2010.1.1-2014.12.31.csv'
    save_path = 'data/beijing/data.csv'
    data_df = pd.read_csv(path)
...
def process_adult():
    path = f'data/adult/adult.data'
    save_path = 'data/adult/data.csv'
...
def process_shoppers():
    path = f'data/shoppers/online_shoppers_intention.csv'
    save_path = 'data/shoppers/data.csv'
    data_df = pd.read_csv(path)
...
def process_default():
    path = f'data/default/default of credit card clients.xls'
    save_path = 'data/default/data.csv'
...
def process_magic():
    ...
    path = f'data/magic/magic04.data'
    save_path = 'data/magic/data.csv'
    data_df = pd.read_csv(path, header=None)
...
def train_test_split(dataname, ratio = 0.7, mask_prob = 0.3):
    ...
    train_y =  train_df.to_numpy()
    test_X = test_df.to_numpy()
    test_y =  test_df.to_numpy()
    
    
    # train_X = train_df.to_numpy()[:, :-1]
    # train_y =  train_df.to_numpy()[:, -1]

    # test_X = test_df.to_numpy()[:, :-1]
    # test_y =  test_df.to_numpy()[:, -1]
    ...
    for cross_idx in range(10):
        ...
        np.save(f'{data_dir}/train_mask_{cross_idx}.npy', mask_train)
        np.save(f'{data_dir}/test_mask_{cross_idx}.npy', mask_test)

    print(dataname, mask_train.shape)
...
if __name__ == '__main__':
    ...
    # process_gesture()
    # train_test_split('gesture', ratio = 0.7, mask_prob = 0.3)
    ...
    # process_magic()
    # train_test_split('magic', ratio = 0.7, mask_prob = 0.3)

    # process_adult()
    # train_test_split('adult', ratio = 0.7, mask_prob = 0.3)
